...
$(document).ready(function(){
    ...
    //     // $(this).val();
    //     var uptType = $("input[name='oh[upt_type]']:checked").val();
    ...
    $(document).on('blur', '.fefal-hide-show', function (e) {
        if ($('.utsac-1').length > 0 || $('.ut-2-sac-1').lenght > 0) {
            var utSac = $('.utsac-1').val();
            var utSac2 = $('.ut-2-sac-1').val();

            $('.fefal-reduction-data').addClass('d-none');
            if ((utSac != '' && utSac >= 11) || (utSac2 != '' && utSac2 >= 11)) {
    ...
    $(document).on('blur','.ut-sac',function(){
        ...
        var week2 = $('.ut-sac-2').val() ? $('.ut-sac-2').val() : 0;
        var dId = $(this).data('id');
    ...
    // new
    $(document).on('click','.married-type',function(){
        marriedType($(this).val());
    });
    ...
    // new
    $(document).on('click','#surgically-type', function(){
        if($(this).is(':checked')){
            $(".surgically-details").removeClass('d-none');
        }else{
            $(".surgically-details").addClass('d-none');
        }
    });
    ...
    function addOrRemoveClass(value,dId){
    ...
    function childData(childNo){
        ...
        for (i = j; i <= childNo; i++) {
            ...
            "<div class='row'><div class='col-md-1'><label class='vertical-form-label pr-0'>H/O :</label></div>"+
                ...
                "<div class='col-md-2'>"+
                    "<div class='radio is-conceived'>"+
                    '<input type=radio name="oh[child][child_data]['+i+'][child_type]" value="single" id="child_type_single_'+i+'"><label for="child_type_single_'+i+'">Single</label>'+
                    '<input type=radio name="oh[child][child_data][' + i + '][child_type]" value="twins" id="child_type_twins_' + i + '"><label for="child_type_twins_' + i + '">Twins</label>' +
                    '<input type=radio name="oh[child][child_data][' + i + '][child_type]" value="triple" id="child_type_triple_' + i + '"><label for="child_type_triple_' + i + '">Triple</label>' +
                    "</div>" +
                    "</div>" +
                    "<div class='col-md-2'>"+
                    "<div class='form-group'>"+
                    // "<label class='vertical-form-label pr-0'>H/O :</label>"+
                    '<select name="oh[child][child_data]['+i+'][ho_gender][]" class="form-control select-padding-0 p-ho-type" title="Select Child Gender" multiple>'+
                            ...
                            "<div class='row col-md-9 d-none mtp-visible-"+i+"'>"+
                                ...
                                "<div class='col-md-8'>"+
                                    "<div class='input-group'>"+
                                        ...
                                        '<input type="text" name="oh[mtp][mtp_data]['+i+'][mtp_month_of_pregancy]" class="form-control">'+
                                    "</div>"+
                                "</div>"+
                            "</div>"+
                        "</div>"+
            ...
            secondMtpData += "<div class='row second-marriage-life-data'>"+
                            ...
                            "<div class='row second-marriage-life-data col-md-9 d-none second-mtp-visible-"+i+"'>"+
                                ...
                                "<div class='col-md-8'>"+
                                    "<div class='input-group'>"+
                                        "<span class='input-group-addon'>Month Of Pregnancy &nbsp;</span>"+
                                        '<input type="text" name="oh[second_marriage][mtp][mtp_data]['+i+'][mtp_month_of_pregancy]" class="form-control">'+
                                    "</div>"+
                                "</div>"+
                            "</div>"+
                        "</div>"+
    ...
    function secondChildData(childNo){
        ...
        for (i = j; i <= childNo; i++) {
            ...
            "<div class='row second-marriage-life-data'><div class='col-md-1'><label class='vertical-form-label pr-0'>H/O :</label></div>"+
                ...
                "<div class='col-md-2'>"+
                "<div class='radio is-conceived'>"+
                '<input type=radio name="oh[second_marriage][child][child_data]['+i+'][child_type]" value="single" id="child_type_single_'+i+'"><label for="child_type_single_'+i+'">Single</label>'+
                '<input type=radio name="oh[second_marriage][child][child_data][' + i + '][child_type]" value="twins" id="child_type_twins_' + i + '"><label for="child_type_twins_' + i + '">Twins</label>' +
                '<input type=radio name="oh[second_marriage][child][child_data][' + i + '][child_type]" value="triple" id="child_type_triple_' + i + '"><label for="child_type_triple_' + i + '">Triple</label>' +
                "</div>" +
                "</div>" +
                "<div class='col-md-2'>"+
                "<div class='form-group'>"+
                '<select name="oh[second_marriage][child][child_data]['+i+'][ho_gender][]" class="form-control select-padding-0 child-ho-type second-p-ho-type" data-id="" title="Select Child Gender" multiple>'+
                '<option value="male">Male</option>'+
                '<option value="female">Female</option>'+
                '</select>'+
                "</div>"+
        ...
        for (i = j; i <= oeValue; i++) {
            oeValueData +=    "<div class='row'>"+
                                    "<div class='col-md-2 ut-g-sac'><div class='radio is-conceived'>"+
                                        "<input type='radio' name='oe[utdata]["+i+"][ut_type]' data-id='"+i+"' value='ut' id='u"+i+"' class='ut-type ut-g-sac-data-"+i+"' checked=true><label for='u"+i+"'>UT(WKS)</label>"+
                                        "<input type='radio' name='oe[utdata]["+i+"][ut_type]' data-id='"+i+"' value='g-sac' id='ug"+i+"' class='ut-type ut-g-sac-data-"+i+"'><label for='ug"+i+"' class='ml-3'>G-sac(MM)</label>"+
                                    "</div></div>"+
                                    "<div class='col-md-1 g-sac ut-g-sac'><div class='form-group'>"+
                                        "<input type='text' name='oe[utdata]["+i+"][oe_ut_sac]' class='form-control crl-number ut-sac max-"+i+" utsac-"+i+" fefal-hide-show g-sac-no' data-id='"+i+"' onwheel='this.blur()' oninput='maxLengthCheck(this)'>"+
                                    "</div></div>"+
                                    "<span class='ut-g-sac-symbol symbol-"+i+"'>-</span>"+
                                    "<div class='col-md-1 g-sac ut-g-sac'><div class='form-group'>"+
                                        "<input type='text' name='oe[utdata]["+i+"][oe_ut_sac_2]' class='form-control crl-number ut-sac-2 max-"+i+" ut-2-sac-"+i+" fefal-hide-show g-sac-no' data-id='"+i+"' onwheel='this.blur()' oninput='maxLengthCheck(this)'>"+
                                    "</div></div>";
                                ...
                                oeValueData += "<div class='col-md-1 pr-0 crl-data-value-"+i+"'><label class='vertical-form-label pr-0'>CRL :</label></div>"+
                                                    "<div class='col-md-2 crl-"+i+" crl-data-value-"+i+"'><div class='form-group'>"+
                                                        "<input type='text' name='oe[utdata]["+i+"][crl]' class='form-control crl-data' data-id="+i+">"+
                                                    "</div></div>"+
                                                    "<div class='col-md-1 p-1 crl-data-value-"+i+"'><span class='crl-text-"+i+"'></span><input type='hidden' name='oe[utdata]["+i+"][crl_details]' class='crl-val-"+i+"'></div>"+
                                                "</div>";
        // if(utType == 'ut'){
    ...
    // new
    function gynecYesNoStatus(value, dType) {
        $('.' + dType).addClass('d-none');
