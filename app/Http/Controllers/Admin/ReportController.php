        ...
        try{
            if($request->ajax())
            {
                ...
                if(!empty($request->plan_type))
                {
                    ...
                    // $data_fail = $this->IuiHistory::where('visit', 4 )->whereIn('patients_id',$data_plan_type)->whereHas('getPatientsDetails')->where('description->result','like', '%fail%')->groupBy('patients_id')->orderBy('id','DESC');
                }
