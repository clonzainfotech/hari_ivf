...
class IVFController extends AdminController
{
    ...
    */
    public function edit($patientsId){
        try{
            ...
            $data['usgImagesData'] = json_encode($usgImagesData,true);
        ...
        try{
            ...
            if($request->isprint){
                ...
                return response()->json([
                    'status' => 1,
                    'id' => $ivfId,
                    'data' => View::make('admin.ivf.preview', compact('pt_view','ivfFirstVisitData','investigationReport','ivf','ivfCycleData', 'historyData', 'isIvfHistory','isTableView','doseData','remark','transferDate','currentdate','lastAppointmentData','ivfSecondVisitData','ohData'))->render()
                ]);
            }
    ...
    */
    public function payment_gujarati(Request $request,$patientsId,$lang){
        ...
        $ivfPaymentList = $this->getIvfPaymentList($patients_Id);
    ...
    public function ivfPayment($patientsId){
        $patientsId = decrypt($patientsId);
        $ivfPaymentList = $this->getIvfPaymentList($patientsId);
            ...
            if($request->ajax()) 
            {
                ...
                if($request->ivf_review_type && ($fromdate || $todate))
                {
                    ...
                    if($ivf_review_type == 2)//result Date
                    {
                        $ivfResultReviewIds = collect($ivfResultReview->get())->map(function ($query) use($fromdate,$todate)
                        {
                            $transferDate = !empty($query->getResult()) ? Carbon::parse($query->getResult()['follow_up'])->format('Y-m-d') : (!empty($query->getTransferDate()['transfer_date']) ? Carbon::parse($query->getTransferDate()['transfer_date'])->addDays(14)->format('Y-m-d') : null);
